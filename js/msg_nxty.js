...
var nxty = {
    ...
    SendNxtyMsg: function( uCmdByte, pMsgData, uLenByte )
    {
      ...
      {
        uTxMsgNotReadyCnt++;
        console.log("Ready Count: "+uTxMsgNotReadyCnt);
...
//                    myModel = "MN" + (nxtyRxStatusBoardConfig | 0x01);    // Model number is now SKU
                }
...
//                                myModel = "MN" + (nxtyRxStatusBoardConfig | 0x01);    // Model number is now SKU
                            ...
                            {
                                PrintLog(1,  "  Connected to CU: Board Config=" + nxtyRxStatusBoardConfig );
                            }
                            else if( nxtyRxStatusBoardConfig & IM_A_1BOX_NU_MASK )
                            {
                                PrintLog(1,  "  Connected to 1BOX NU: Board Config=" + nxtyRxStatusBoardConfig );
                            }
                            else
                            {
                                PrintLog(1,  "  Connected to NU: Board Config=" + nxtyRxStatusBoardConfig );
                            }
                            ...
                            else
                            {
                               ...
                               //console.log("REquired Registration");
                            }
...
function WriteAddrReq( u32Addr, u32Data )
{
    ...
    if( u32Addr == NXTY_PCCTRL_CLOUD_INFO )
    {
        if( bCnxToCu && (IsUartRemote() == false) )
        {
            // Cloud info to CU
            if( u32Data & 0x0200 )  outText += "  GetPage";     // Bit 9
            if( u32Data & 0x0800 )  outText += "  Label";       // Bit 11 
            if( u32Data & 0x00FF )  outText += "  Num:" + (u32Data & 0x00FF);     // Bits 0 to 7
        }
        else
        {
            // Cloud info to NU
            if( u32Data & 0x0004 )  outText += "  AntUpdate";       // Bit 2 
            if( u32Data & 0x0002 )  outText += "  RegReq";          // Bit 1
            if( u32Data & 0x0001 )  outText += "  CellInfoReq";     // Bit 0 
        }     
    }
    else if( u32Addr == NXTY_PCCTRL_GLOBALFLAGS )
    {
